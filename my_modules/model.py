...
from my_modules.logger import update_predict_log, update_train_log
from my_modules.data_utils import fetch_ts, engineer_features
...
def _model_train(df, tag, test=False):
	...
	if test:
		...
		saved_model = os.path.join(MODEL_DIR, "prod-{}-{}.joblib".format(tag, model_name))
	...
	## update log
	update_train_log(tag, (str(dates[0]), str(dates[-1])), {'rmse': eval_rmse}, runtime, MODEL_VERSION, MODEL_VERSION_NOTE, test)
...
def model_train(data_dir=None, test=False):
	...
	for country, df in ts_data.items():
		...
		_model_train(df, country, test)

# funtion to load model
def model_load(data_dir=None,training=True, test=False):
	if test:
		prefix = 'test'
	else:
		prefix = 'prod'
...
def model_predict(country, year, month, day, all_data=None, all_models=None, test=False):
	"""
	...
	if not all_models or not all_data:
